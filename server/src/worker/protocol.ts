...
// page_speech (§22): text of a page punctuated for the voice (« Préparer la lecture »). 'page_audio' reserved.
export type WorkerJobKind = 'ai' | 'page_text' | 'page_speech';
...
export interface LeasedWorkerJob extends WorkerJobRequest {
  ...
  tier: WorkerTier;
  operation: string;
  /** page_text: '/api/worker/jobs/<id>/image' (null for ai and page_speech). */
  imageUrl: string | null;
  /** Maximum execution time on the worker. */
  deadlineMs: number;
  leaseMs: number;
  /** How much the model may reflect before answering ('off': answer directly). Ignored by workers older than 1.2. */
  reasoning: WorkerReasoning;
}

export type WorkerReasoning = 'off' | 'low' | 'medium' | 'high';

export interface WorkerLimits {
  status: 'allowed' | 'allowed_warning' | 'rejected';
  rateLimitType: string | null;
  /** Share of the subscription window used, in % (0–100). */
  utilization: number | null;
  /** Epoch ms. */
  resetsAt: number | null;
  /** §21: when the worker saw these limits (epoch ms, worker clock); absent from older workers. */
...
export type ResultRequest =
  | {
    ...
    inputTokens: number | null; outputTokens: number | null; durationMs: number; costMicros?: number | null;
  }
  | { worker: string; outcome: 'error'; error: WorkerErrorCode; retryAfterMs: number | null; message: string; costMicros?: number | null };
// costMicros (§21): estimated cost of the run in millionths of a euro, worker side (the server holds no price list).
...
export interface TranscriptionsRequest { documentId: string; pageIndexes?: number[]; reread?: boolean }
...
export const WORKER_PROTOCOL = {
  ...
  /**
   * §17.4: time an `ai` job may spend waiting in the queue, on top of its execution budget. A worker leases one job at
   * a time, so a request sent while another one runs (the progressive summary sends several chunks) must not spend its
   * own budget queueing: `worker.deadlines[tier]` stays the budget of one run, this is the wait allowed before it.
   */
  queueGraceMs: 120_000,
  imagesMaxBytes: 2 * 1024 * 1024,
  messageMaxChars: 300,
} as const;

export const WORKER_JOB_KINDS = ['ai', 'page_text', 'page_speech'] as const satisfies readonly WorkerJobKind[];
...
/** At most 100 € for one run. */
const CostMicrosSchema = z.number().int().nonnegative().max(100_000_000).nullable().optional();
...
export const WorkerLimitsSchema = z.object({
  ...
  observedAt: z.number().int().nonnegative().nullable().optional(),
});
