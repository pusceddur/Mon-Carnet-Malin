...
function toBytes(data: string | Uint8Array | ArrayBuffer): Uint8Array<ArrayBuffer> {
...
/** Pure JS SHA-256 (used when crypto.subtle is unavailable, e.g. insecure context). */
...
export async function sha256Hex(data: string | Uint8Array | ArrayBuffer): Promise<string> {
  const subtle = (globalThis as { crypto?: { subtle?: { digest(algorithm: string, data: Uint8Array<ArrayBuffer>): Promise<ArrayBuffer> } } }).crypto?.subtle;
