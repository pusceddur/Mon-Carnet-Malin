...
import { READING_FONTS } from '../../design/reading';
import { format } from '../../i18n/fr';
import { aids as aidStrings } from '../../i18n/fr/aids';
import { common } from '../../i18n/fr/common';
import { parent } from '../../i18n/fr/parent';
import { READING_AID_KEYS, readingAidsOf } from '../reader/aids';
import { ReadingPreview } from '../reader/ReadingPreview';
...
function ChildForm({ childId }: { childId: string | null }): JSX.Element {
  ...
  const profile = matchReadingProfile(values.reading, values.tts);
  const aids = readingAidsOf(values.reading);
  ...
  const save = async (): Promise<void> => {
    ...
    setErrors(found);
    // The question types are in the advanced settings: open them to show the error.
    if (found.questionTypes) setAdvanced(true);
  ...
  return (
    <ParentPage title={title} intro={t.privacyNote}>
      ...
      <ParentSection title={t.sections.profile} hint={t.profiles.hint}>
        <div role="radiogroup" aria-label={t.sections.profile} className="profile-grid">
          {READING_PROFILES.map((item) => (
            <button
              ...
              onClick={() => setValues((v) => ({ ...v, ...applyReadingProfile(v, item) }))}
            >
              <span className="profile-choice__emoji" aria-hidden="true">{item.emoji}</span>
              <span className="profile-choice__name">{t.profiles.items[item.id].name}</span>
              <span className="profile-choice__description">{t.profiles.items[item.id].description}</span>
            </button>
          ))}
        </div>
        ...
        <div className="stack stack--tight">
          ...
          <div className="reading-preview" data-theme={values.reading.theme}>
            <ReadingPreview text={t.previewText} reading={values.reading} />
          </div>
        </div>
      </ParentSection>
      ...
      <AdvancedSettings label={t.advanced} hint={t.advancedHint} open={advanced} onToggle={() => setAdvanced((open) => !open)}>
        <ParentSection title={t.explanationDifficulty} hint={t.explanationHint}>
          <Segmented
        ...
        <ParentSection title={aidStrings.title} hint={aidStrings.hint}>
          {READING_AID_KEYS.map((key) => (
            <Toggle
              ...
              checked={aids[key]}
              onChange={(on) => setReading('aids', { ...aids, [key]: on })}
            />
          ))}
        </ParentSection>
